...
void imprime_grafo(Grafo *gr){
    ...
    for(i=0; i<gr->num_vertices; i++){
        ...
        while (no->prox_no != NULL) {
            printf("%s -> %s | Num Grau: %d | Num Arestas: %d | Peso: %d\n", gr->raiz[i].rotulo, no->prox_no->vertice->rotulo, gr->raiz[i].num_graus, gr->raiz[i].num_aresta, no->peso);
            no = no->prox_no;
        }
        printf("\n");
    }
